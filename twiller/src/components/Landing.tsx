...
export default function LandingPage() {
  ...
  const [prefilledOtpEmail, setPrefilledOtpEmail] = useState("");
  const [showOtpOnOpen, setShowOtpOnOpen] = useState(false);
  const { user, logout, googlesignin } = useAuth();

  const openAuthModal = (mode: "login" | "signup") => {
    setAuthMode(mode);
    setPrefilledOtpEmail("");
    setShowOtpOnOpen(false);
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      if (res?.requiresOtp) {
        setPrefilledOtpEmail(res.email || "");
        setShowOtpOnOpen(true);
        setAuthMode("login");
        setShowAuthModal(true);
      }
    } catch (err: any) {
  ...
  return (
    <div className="min-h-screen bg-black text-white flex">
      ...
      <AuthModal
        ...
        prefilledOtpEmail={prefilledOtpEmail}
        showOtpOnOpen={showOtpOnOpen}
      />
    </div>
  );
}
